...
impl DigikeyClient {
    ...
    async fn cached_api_request(
        ...
        // Try cache first
        match self.api_cache.get(method, DIGIKEY_API_BASE, path, path_params, query_params, force_refresh) {
            Ok(cached_response) => {
    ...
    pub async fn get_manufacturers_cached(&self, force_refresh: bool) -> Result<ManufacturersResponse, DigikeyError> {
        let response_text = self.cached_api_request(
            "GET",
        ...
        info!("Fetched {} manufacturers (from cache or API)", manufacturers.manufacturers.len());
        Ok(manufacturers)
    }

    /// Get part details using cache
    pub async fn get_part_details_cached(&self, product_number: &str, force_refresh: bool) -> Result<PartDetails, DigikeyError> {
        ...
        let detailed_description = product.product
            .description
        ...
        let product_status = product.product
            .product_status
            .as_ref()
    ...
    pub async fn update_manufacturers(force_refresh: bool) -> Result<(), DigikeyError> {
        debug!("Updating manufacturers data (force_refresh={})", force_refresh);
    ...
    pub async fn write_parts_to_kdl(&self, parts_with_details: &[(String, PartDetails)]) -> Result<PathBuf, DigikeyError> {
        ...
        for (mpn, details) in parts_with_details {
            ...
            if let Some(price) = details.unit_price {
                kdl_content.push_str(&format!("        pricing currency=\"USD\" unit={price:.2}\n"));
            }
    ...
    pub async fn update_parts(force_refresh: bool) -> Result<(), DigikeyError> {
        ...
        println!("Fetching {} parts from Digikey API (with caching)", all_mpns.len());
        ...
        println!("Writing {} parts to db/digikey/part-details.kdl", all_parts_with_details.len());
